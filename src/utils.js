...
export function runSequences(arr) {
  return arr.reduce((promise, func) => promise.then(func), Promise.resolve());
}
export function getArrTime() {
  const time = new Date(Date.now());
  const timeToString = time.toTimeString().slice(0, 5).replace(':', '');
...
export const findClock = (numbers, conditionFunc) =>
